                    ...
                    <table class="table table-bordered table-sm table-head-fixed">
                      <thead>
                        <tr>
                          ...
                          <th class="text-center" width="200"><?= table_sort($menu['menu_id'], 'Judul Galeri', 'galeri_title', $cookie['order']) ?></th>
                            ...
                            foreach ($main as $r) : ?>
                              <tr>
                                ...
                                <td class="text-center">
                                  <div class="pretty p-icon">
                                    <input class="checkitem" type="checkbox" value="<?= $r['galeri_id'] ?>" name="checkitem[]" onclick="checkItem();" />
                                    <div class="state">
                                      <i class="icon fas fa-check"></i><label></label>
                                    </div>
                                  </div>
                                ...
                                <td class="text-center">
                                  <?php if ($menu['_update'] == 1) : ?>
                                    <a class="text-warning mr-1" href="<?= site_url() . '/' . $menu['controller'] . '/form/' . $r['galeri_id'] ?>"><i class="fas fa-pencil-alt"></i></a>
                                  <?php endif; ?>
                                  <?php if ($menu['_delete'] == 1) : ?>
                                    <a class="text-danger btn-delete" href="<?= site_url() . '/' . $menu['controller'] . '/delete/' . $r['galeri_id'] ?>"><i class="fas fa-trash-alt"></i></a>
                                  <?php endif; ?>
                                </td>
                                <td><?= $r['galeri_title'] ?></td>
                                <td class="text-center"><?= to_date($r['galeri_date'],'','full_date') ?></td>
                                <td><img src="<?= base_url() ?>images/galeri/<?= @$r['galeri_image'] ?>" alt="<?= @$r['galeri_image'] ?>" class="img-thumbnail"></td>
                                <td class="text-center td-status">
                                  <?php if ($menu['_update'] == 1) : ?>
                                    <?php if ($r['active_st'] == 1) : ?>
                                      <a href="<?= site_url() . '/' . $menu['controller'] . '/status/disable/' . $r['galeri_id'] ?>">
                                        <i class="icon-status fas fa-toggle-on text-success"></i>
                                      </a>
                                    <?php else : ?>
                                      <a href="<?= site_url() . '/' . $menu['controller'] . '/status/enable/' . $r['galeri_id'] ?>">
