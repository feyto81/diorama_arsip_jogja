...
class Galeri extends MY_Controller
{
  ...
  function __construct()
  {
    ...
    if ($this->cookie['order'] == null) $this->cookie['order'] = array('field' => 'galeri_title', 'type' => 'asc');
  ...
  public function index()
  {
    ...
    $this->cookie['total_rows'] = $this->m_galeri->all_rows($this->cookie);
    set_cookie_menu($this->menu_id, $this->cookie);
    //main data
    $data['menu'] = $this->menu;
    $data['cookie'] = $this->cookie;
    $data['main'] = $this->m_galeri->list_data($this->cookie);
  ...
  public function form($id = null)
  {
    ...
    if ($id == null) {
      ...
      $data['main'] = $this->m_galeri->by_field('galeri_id', $id);
    }
    $data['id'] = $id;
    $data['menu'] = $this->menu;
  ...
  public function save($id = null)
  {
    ...
    unset($data['files'], $data['old_galeri_image']);
    if (!isset($data['active_st'])) {
      $data['active_st'] = 0;
    }
    $data['galeri_image'] = $this->_uploadImage();
    $data['galeri_date'] = to_date($data['galeri_date'], '', 'full_date');
    if ($id == null) {
      $cek = $this->m_galeri->by_field('galeri_title', $data['galeri_title']);
      if ($cek != null) {
        $this->session->set_flashdata('flash_error', 'Nama sudah ada di sistem.');
        redirect(site_url() . '/' .  $this->menu['controller']  . '/form/');
      }
      $data['galeri_id'] = $this->uuid->v4();
      $this->m_galeri->save($data, $id);
      create_log('fb49e2c1-a762-4a34-8e2c-c8fab48814f4', $this->menu['menu_name']);
      $this->session->set_flashdata('flash_success', 'Data berhasil ditambahkan.');
    } else {
      if ($data['old'] != $data['galeri_id'] && $cek != null) {
        $this->session->set_flashdata('flash_error', 'Nama sudah ada di sistem.');
  ...
  {
    $config['upload_path']          = FCPATH . '/images/galeri/';
  ...
  public function ajax($type = null, $id = null)
  {
    if ($type == 'check_id') {
      ...
      $cek = $this->m_galeri->by_field('galeri_title', $data['galeri_title']);
      $old = $this->m_galeri->by_field('galeri_id', $id);
      if ($id == null) {
        echo ($cek != null) ? 'false' : 'true';
      } else {
        echo ($cek != null) ? (($cek['galeri_title'] == $old['galeri_title']) ? 'true' : 'false') : 'true';
      }
    }
  }
}
