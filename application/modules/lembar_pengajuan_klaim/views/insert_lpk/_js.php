<script type="text/javascript">
  ...
  $(document).ready(function() {
    var form_submit = $("#form_submit").validate({
      ...
      submitHandler: function(form) {
        ...
        $.ajax({
          type: "POST",
          url: "<?= site_url() . '/' . $menu['controller'] . '/insert_lpk/process' ?>",
          data: $(form).serialize(),
  ...
  function add_diagnosa(no) {
    ...
    $(".btn-add-diagnosa").html('<i class="fas fa-spin fa-spinner"></i> Proses');
    $(".btn-add-diagnosa").addClass('disabled');
    $.post('<?= site_url() . '/' . $menu['controller'] . '/insert_lpk/ajax/add_diagnosa' ?>', {
      diagnosa_no: no
    }, function(data) {
  ...
  function add_procedure(no) {
    ...
    $(".btn-add-procedure").html('<i class="fas fa-spin fa-spinner"></i> Proses');
    $(".btn-add-procedure").addClass('disabled');
    $.post('<?= site_url() . '/' . $menu['controller'] . '/insert_lpk/ajax/add_procedure' ?>', {
      procedure_no: no
    }, function(data) {
