                    ...
                    <table class="table table-bordered table-sm table-head-fixed">
                      <thead>
                        <tr>
                          ...
                          <th class="text-center" width="60"><?= table_sort($menu['menu_id'], 'Kode', 'menu_id', $cookie['order']) ?></th>
                          <th class="text-center" width="60">Induk</th>
                          <th class="text-center">Type</th>
                          <th class="text-center"><?= table_sort($menu['menu_id'], 'Nama', 'menu_name', $cookie['order']) ?></th>
                          <th class="text-center">Icon</th>
                          <th class="text-center">Controller</th>
                          <th class="text-center">Url</th>
                          <th class="text-center" width="30"><i class="fas fa-eye"></i></th>
                          <th class="text-center" width="30"><i class="fas fa-plus-circle"></i></th>
                            ...
                            foreach ($main as $r) : ?>
                              <tr>
                                ...
                                <td class="text-center">
                                  <div class="pretty p-icon">
                                    <input class="checkitem" type="checkbox" value="<?= $r['menu_id'] ?>" name="checkitem[]" onclick="checkItem();" />
                                    <div class="state">
                                      <i class="icon fas fa-check"></i><label></label>
                                    </div>
                                  </div>
                                ...
                                <td class="text-center">
                                  <?php if ($menu['_update'] == 1) : ?>
                                    <a class="text-warning mr-1" href="<?= site_url() . '/' . $menu['controller'] . '/form/' . $r['menu_id'] ?>"><i class="fas fa-pencil-alt"></i></a>
                                  <?php endif; ?>
                                  <?php if ($menu['_delete'] == 1) : ?>
                                    <a class="text-danger btn-delete" href="<?= site_url() . '/' . $menu['controller'] . '/delete/' . $r['menu_id'] ?>"><i class="fas fa-trash-alt"></i></a>
                                  <?php endif; ?>
                                </td>
                                <td><?= $r['menu_id'] ?></td>
                                <td><?= $r['parent_id'] ?></td>
                                ...
                                <td>
                                  <?php
                                  $div = intval(strlen($r['menu_id']) / 5);
                                  $strip = '';
                                  $margin = 0;
                                  if ($div != 0) {
                                    $strip .= '|--';
                                    $l = strlen($r['menu_id']) - 5;
                                    for ($j = 0; $j < $l; $j++) {
                                      $margin += 5;
                                    }
                                  }
                                  ?>
                                  <span style="margin-left: <?= $margin ?>px"></span><?= $strip . " " . $r['menu_name'] ?>
                                </td>
                                <td class=""><?= $r['icon'] ?></td>
                                <td class=""><?= $r['controller'] ?></td>
                                <td class=""><?= $r['url'] ?></td>
                                <td class="text-center">
                                  <i class="fas fa-circle text-<?= ($r['is_read'] == 1) ? 'success' : 'gray' ?>"></i>
                                </td>
                                <td class="text-center">
                                  <i class="fas fa-circle text-<?= ($r['is_create'] == 1) ? 'success' : 'gray' ?>"></i>
                                </td>
                                <td class="text-center">
                                  <i class="fas fa-circle text-<?= ($r['is_update'] == 1) ? 'success' : 'gray' ?>"></i>
                                </td>
                                <td class="text-center">
                                  <i class="fas fa-circle text-<?= ($r['is_delete'] == 1) ? 'success' : 'gray' ?>"></i>
                                </td>
                                <td class="text-center">
                                  <i class="fas fa-circle text-<?= ($r['is_report'] == 1) ? 'success' : 'gray' ?>"></i>
                                </td>
                                <td class="text-center td-status">
                                  <?php if ($menu['_update'] == 1) : ?>
                                    ...
                                    <?php else : ?>
                                      <a href="<?= site_url() . '/' . $menu['controller'] . '/status/enable/' . $r['menu_id'] ?>">
